...
void sqliteDropTable(Parse *pParse, Token *pName){
  ...
  if( v ){
    ...
    for(pIdx=pTable->pIndex; pIdx; pIdx=pIdx->pNext){
      sqliteVdbeAddOp(v, 11, 0, 0, pIdx->zName, 0);
    }
  }
...
void sqliteCreateIndex(
  ...
  */
  if( pParse->initFlag==0 ){
    ...
    sqliteVdbeAddOp(v, 10, 0, lbl2, 0, lbl1);
    sqliteVdbeAddOp(v, 8, 0, 0, 0, 0);
    for(i=0; i<pIndex->nField; i++){
      sqliteVdbeAddOp(v, 7, 0, pIndex->aiField[i], 0, 0);
    }
    sqliteVdbeAddOp(v, 36, pIndex->nField, 0, 0, 0);
    sqliteVdbeAddOp(v, 15, 1, 0, 0, 0);
    sqliteVdbeAddOp(v, 37, 0, lbl1, 0, 0);
    sqliteVdbeAddOp(v, 70, 0, 0, 0, lbl2);
...
void sqliteInsert(
  ...
  if( v ){
    ...
    sqliteVdbeAddOp(v, 35, pTab->nCol, 0, 0, 0);
...
void sqliteSelect(
  ...
  */
  sqliteVdbeAddOp(v, 40, pEList->nExpr, 0, 0, 0);
  for(i=0; i<pEList->nExpr; i++){
    Expr *p;
    if( pEList->a[i].zName ){
      char *zName = pEList->a[i].zName;
      int addr = sqliteVdbeAddOp(v, 41, i, 0, zName, 0);
      if( zName[0]=='\'' || zName[0]=='"' ){
    ...
    if( p->op!=TK_FIELD ){
      ...
      sqliteVdbeAddOp(v, 41, i, 0, zName, 0);
    }else{
      if( pTabList->nId>1 ){
        char *zName = 0;
        ...
        sqliteVdbeAddOp(v, 41, i, 0, pTab->azCol[p->iField], 0);
      }
    }
  }
  ...
  */
  if( pOrderBy==0 ){
    sqliteVdbeAddOp(v, 42, pEList->nExpr, 0, 0, 0);
  }else{
    char *zSortOrder;
    sqliteVdbeAddOp(v, 24, pEList->nExpr, 0, 0, 0);
    ...
    sqliteVdbeAddOp(v, 25, pOrderBy->nExpr, 0, zSortOrder, 0);
    sqliteVdbeAddOp(v, 23, 0, 0, 0, 0);
  }
  ...
  */
  if( pOrderBy ){
    ...
    int addr;
    sqliteVdbeAddOp(v, 26, 0, 0, 0, 0);
    addr = sqliteVdbeAddOp(v, 27, 0, end, 0, 0);
    sqliteVdbeAddOp(v, 29, pEList->nExpr, 0, 0, 0);
    sqliteVdbeAddOp(v, 37, 0, addr, 0, 0);
    sqliteVdbeAddOp(v, 70, 0, 0, 0, end);
  }
...
void sqliteUpdate(
  ...
  */
  for(i=0; i<nIdx; i++){
    sqliteVdbeAddOp(v, 46, pTab->nCol, 0, 0, 0); /* The KEY */
    pIdx = apIdx[i];
    for(j=0; j<pIdx->nField; j++){
      sqliteVdbeAddOp(v, 46, j+pTab->nCol-pIdx->aiField[j], 0, 0, 0);
    }
    sqliteVdbeAddOp(v, 36, pIdx->nField, 0, 0, 0);
...
void sqliteCopy(
  ...
  v = pParse->pVdbe = sqliteVdbeCreate(pParse->db->pBe);
  if( v ){
    addr = sqliteVdbeAddOp(v, 31, 0, 0, 0, 0);
    ...
    addr = sqliteVdbeAddOp(v, 32, pTab->nCol, end, 0, 0);
    if( pDelimiter ){
    ...
    for(i=1, pIdx=pTab->pIndex; pIdx; pIdx=pIdx->pNext, i++){
      ...
      for(j=0; j<pIdx->nField; j++){
        sqliteVdbeAddOp(v, 33, pIdx->aiField[j], 0, 0, 0);
      }
      sqliteVdbeAddOp(v, 36, pIdx->nField, 0, 0, 0);
...
void sqliteVacuum(Parse *pParse, Token *pTableName){
  ...
  if( zName ){
    sqliteVdbeAddOp(v, 12, 0, 0, zName, 0);
  }else{
    int h;
    Table *pTab;
    Index *pIdx;
    for(h=0; h<51; h++){
      for(pTab=pParse->db->apTblHash[h]; pTab; pTab=pTab->pHash){
        sqliteVdbeAddOp(v, 12, 0, 0, pTab->zName, 0);
        for(pIdx=pTab->pIndex; pIdx; pIdx=pIdx->pNext){
