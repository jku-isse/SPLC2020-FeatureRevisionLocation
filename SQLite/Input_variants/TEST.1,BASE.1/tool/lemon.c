...
#define MemoryCheck(X) if((X)==0){ extern void memory_error(); memory_error(); }
...
struct action *ap2;
{
  ...
  if( rc==0 ){
    if(!(ap1->type==REDUCE && ap2->type==REDUCE))myassert("C:\\Users\\gabil\\Desktop\\ECCO_Work\\spls\\spls\\tests-11-02-2020\\SQLite\\sqlite\\tool\\lemon.c",66);
...
*/
 struct state *getstate(/* struct lemon * */);  /* forward reference */
...
struct lemon *lemp;
{
  ...
  ** start nonterminal.  */
  ...
  for(i=0; i<lemp->nstate; i++){
    ...
    struct state *stp;
    stp = lemp->sorted[i];
    if(!(stp->ap))myassert("C:\\Users\\gabil\\Desktop\\ECCO_Work\\spls\\spls\\tests-11-02-2020\\SQLite\\sqlite\\tool\\lemon.c",66);
...
struct symbol *errsym;   /* The error symbol (if defined.  NULL otherwise) */
{
  ...
  int errcnt = 0;
  if(!(apx->sp==apy->sp))myassert("C:\\Users\\gabil\\Desktop\\ECCO_Work\\spls\\spls\\tests-11-02-2020\\SQLite\\sqlite\\tool\\lemon.c",66);  /* Otherwise there would be no conflict */
  if( apx->type==SHIFT && apy->type==REDUCE ){
    ...
    if( spy==0 || spx->prec<0 || spy->prec<0 ){
      ...
      if(!(spx->prec==spy->prec && spx->assoc==NONE))myassert("C:\\Users\\gabil\\Desktop\\ECCO_Work\\spls\\spls\\tests-11-02-2020\\SQLite\\sqlite\\tool\\lemon.c",66);
      apy->type = CONFLICT;
      errcnt++;
    }
...
int dot;            /* Index into the RHS of the rule where the dot goes */
{
  ...
  if(!(currentend!=0))myassert("C:\\Users\\gabil\\Desktop\\ECCO_Work\\spls\\spls\\tests-11-02-2020\\SQLite\\sqlite\\tool\\lemon.c",66);
  model.rp = rp;
  model.dot = dot;
...
int dot;
{
  ...
  if(!(basisend!=0))myassert("C:\\Users\\gabil\\Desktop\\ECCO_Work\\spls\\spls\\tests-11-02-2020\\SQLite\\sqlite\\tool\\lemon.c",66);
  if(!(currentend!=0))myassert("C:\\Users\\gabil\\Desktop\\ECCO_Work\\spls\\spls\\tests-11-02-2020\\SQLite\\sqlite\\tool\\lemon.c",66);
  model.rp = rp;
  model.dot = dot;
  cfp = Configtable_find(&model);
...
struct config *cfp;
{
  ...
  for(; cfp; cfp=nextcfp){
    nextcfp = cfp->next;
    if(!(cfp->fplp==0))myassert("C:\\Users\\gabil\\Desktop\\ECCO_Work\\spls\\spls\\tests-11-02-2020\\SQLite\\sqlite\\tool\\lemon.c",66);
    if(!(cfp->bplp==0))myassert("C:\\Users\\gabil\\Desktop\\ECCO_Work\\spls\\spls\\tests-11-02-2020\\SQLite\\sqlite\\tool\\lemon.c",66);
...
int offset;
{
  ...
  if( a==0 ){
    ...
    while( a && b ){
      if( (*cmp)(a,b)<0 ){
        (*(char**)(((int)ptr)+offset)) = a;
        ptr = a;
        a = (*(char**)(((int)a)+offset));
      }else{
        (*(char**)(((int)ptr)+offset)) = b;
        ptr = b;
        b = (*(char**)(((int)b)+offset));
      }
    }
    if( a ) (*(char**)(((int)ptr)+offset)) = a;
    else    (*(char**)(((int)ptr)+offset)) = b;
  }
  return head;
}
...
int (*cmp)();
{
  ...
  while( list ){
    ep = list;
    list = (*(char**)(((int)list)+offset));
    (*(char**)(((int)ep)+offset)) = 0;
  ...
  ep = 0;
  for(i=0; i<30; i++) if( set[i] ) ep = merge(ep,set[i],cmp,offset);
  return ep;
}
...
struct pstate {
  ...
  struct symbol *rhs[5];  /* RHS symbols */
  char *alias[5];       /* Aliases for each RHS symbol (or NULL) */
...
"There is not prior rule opon which to attach the code fragment which begins on this line.");
...
** function.
*/
 char *file_makename(lemp,suffix)
struct lemon *lemp;
char *suffix;
{
  char *name;
...
** to the stream */
 FILE *file_open(lemp,suffix,mode)
struct lemon *lemp;
char *suffix;
char *mode;
{
...
struct lemon *lemp;
{
  ...
  for(i=0; i<skip; i++){
    ...
    for(j=i; j<lemp->nsymbol; j+=skip){
      sp = lemp->symbols[j];
      if(!(sp->index==j))myassert("C:\\Users\\gabil\\Desktop\\ECCO_Work\\spls\\spls\\tests-11-02-2020\\SQLite\\sqlite\\tool\\lemon.c",66);
...
** the exacutable */
 char *pathsearch(argv0,name,modemask)
char *argv0;
char *name;
int modemask;
{
...
*/
 void tplt_xfer(name,in,out,lineno)
char *name;
FILE *in;
FILE *out;
int *lineno;
...
/* Print a string to the file and keep the linenumber up to date */
 void tplt_print(out,lemp,str,strln,lineno)
FILE *out;
struct lemon *lemp;
char *str;
int strln;
...
*/
 void emit_code(out,rp,lemp,lineno)
FILE *out;
struct rule *rp;
struct lemon *lemp;
int *lineno;
{
 ...
 char used[5];   /* True for each RHS element which is used */
...
"Illegal stack size: [%s].  The stack size should be an integer constant.",
  ...
  */
  ...
  for(i=0; i<lemp->nstate; i++){
    ...
    if(!(tablesize<= sizeof(table)/sizeof(table[0])))myassert("C:\\Users\\gabil\\Desktop\\ECCO_Work\\spls\\spls\\tests-11-02-2020\\SQLite\\sqlite\\tool\\lemon.c",66);
...
struct lemon *lemp;
{
  ...
  for(i=0; i<lemp->nstate; i++){
    ...
    for(ap=stp->ap; ap && ap->type!=REDUCE; ap=ap->next);
    if(!(ap))myassert("C:\\Users\\gabil\\Desktop\\ECCO_Work\\spls\\spls\\tests-11-02-2020\\SQLite\\sqlite\\tool\\lemon.c",66);
...
struct state *State_new()
{
  ...
  if((new)==0){ extern void memory_error(); memory_error(); };
  return new;
}
