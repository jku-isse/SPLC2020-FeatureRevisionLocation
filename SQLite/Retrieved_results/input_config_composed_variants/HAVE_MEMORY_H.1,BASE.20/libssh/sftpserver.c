...
int sftp_reply_name(SFTP_CLIENT_MESSAGE *msg, char *name, SFTP_ATTRIBUTES *attr){
    ...
    r=sftp_packet_write(msg->sftp,104,out);
    buffer_free(out);
    return r<0;
}
...
int sftp_reply_handle(SFTP_CLIENT_MESSAGE *msg, STRING *handle){
    ...
    r=sftp_packet_write(msg->sftp,102,out);
    buffer_free(out);
    return r<0;
}
...
int sftp_reply_attr(SFTP_CLIENT_MESSAGE *msg, SFTP_ATTRIBUTES *attr){
    ...
    buffer_add_u32(out,msg->id);
    buffer_add_attributes(out,attr);
    r=sftp_packet_write(msg->sftp,105,out);
    buffer_free(out);
    return r<0;
}
...
int sftp_reply_status(SFTP_CLIENT_MESSAGE *msg, u32 status, char *message){
    ...
    r=sftp_packet_write(msg->sftp,101,out);
    buffer_free(out);
    return r<0;
}
...
int sftp_reply_data(SFTP_CLIENT_MESSAGE *msg, void *data, int len){
    ...
    r=sftp_packet_write(msg->sftp,103,out);
    buffer_free(out);
    return r<0;
}
...
STRING *sftp_handle_alloc(SFTP_SESSION *sftp, void *info){
    ...
    STRING *ret;
    if(!sftp->handles){
        sftp->handles=malloc(sizeof(void *) * 256);
        memset(sftp->handles,0,sizeof(void *)*256);
    }
    for(i=0; i<256;++i)
        if(!sftp->handles[i])
            break;
