...
#define BLOCK(letter, n) do { ptr=strchr(alphabet,source[n]); if(!ptr) return -1; i=ptr-alphabet; ##SET_letter(*block,i); } while(0)
...
static int to_block4(unsigned long *block, char *source,int num){
    ...
    *block=0;
    if(num<1)
        return 0;
    do { ptr=strchr(alphabet,source[0]); if(!ptr) return -1; i=ptr-alphabet; do { *block |= (i&63) <<18; } while (0); } while(0); /* 6 bits */
    do { ptr=strchr(alphabet,source[1]); if(!ptr) return -1; i=ptr-alphabet; do { *block |= (i&63) <<12; } while (0); } while(0); /* 12 */
    if(num<2)
        return 0;
    do { ptr=strchr(alphabet,source[2]); if(!ptr) return -1; i=ptr-alphabet; do { *block |= (i&63) << 6; } while (0); } while(0); /* 18 */
    if(num < 3)
        return 0;
    do { ptr=strchr(alphabet,source[3]); if(!ptr) return -1; i=ptr-alphabet; do { *block |= (i&63); } while (0); } while(0); /* 24 */
    return 0;
}
...
static int _base64_to_bin(unsigned char dest[3], char *source,int num){
   ...
   if(to_block4(&block,source,num))
       return -1;
   dest[0]=((block & 0xff0000) >> 16);
   dest[1]=((block & 0xff00) >> 8);
   dest[2]=(block & 0xff);
   return 0;
}
...
static void _bin_to_base64(unsigned char *dest, unsigned char source[3], int len){
    switch (len){
        case 1:
            ...
            dest[1]=alphabet[((source[0] & ((1<<2)-1)) << 4)];
            dest[2]='=';
            dest[3]='=';
            break;
        case 2:
            dest[0]=alphabet[source[0]>>2];
            dest[1]=alphabet[(source[1]>>4) | ((source[0] & ((1<<2)-1)) << 4)];
            dest[2]=alphabet[(source[1]&((1<<4)-1)) << 2]; 
            dest[3]='=';
            break;
        case 3:
            dest[0]=alphabet[(source[0]>>2)]; 
            dest[1]=alphabet[(source[1]>>4) | ((source[0] & ((1<<2)-1)) << 4)];
            dest[2]=alphabet[ (source[2] >> 6) | (source[1]&((1<<4)-1)) << 2]; 
            dest[3]=alphabet[source[2]&((1<<6)-1)];
        break;
    }
}
