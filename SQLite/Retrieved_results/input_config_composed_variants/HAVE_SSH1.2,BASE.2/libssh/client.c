...
char *ssh_get_banner(SSH_SESSION *session){
    ...
    ssh_set_error(NULL,2,"Too large banner");
    return NULL;
}
...
int ssh_analyze_banner(SSH_SESSION *session, int *ssh1, int *ssh2){
    ...
    if(strncmp(banner,"SSH-",4)!=0){
        ssh_set_error(NULL,2,"Protocol mismatch: %s",banner);
        return -1;
    }
    /* a typical banner is :
     * SSH-1.5-blah
...
int dh_handshake(SSH_SESSION *session){
    ...
    if(!pubkey){
        ssh_set_error(NULL,2,"No public key in packet");
        return -1;
    }
    dh_import_pubkey(session,pubkey);
    f=buffer_get_ssh_string(session->in_buffer);
    if(!f){
        ssh_set_error(NULL,2,"No F number in packet");
        return -1;
    }
    dh_import_f(session,f);
    free(f);
    if(!(signature=buffer_get_ssh_string(session->in_buffer))){
        ssh_set_error(NULL,2,"No signature in packet");
        return -1;
    }
