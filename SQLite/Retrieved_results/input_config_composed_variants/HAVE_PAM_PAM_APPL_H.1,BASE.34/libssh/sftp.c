...
int sftp_server_init(SFTP_SESSION *sftp){
    ...
    if(!packet)
        return -1;
    if(packet->type != 1){
        ssh_set_error(sftp->session,2,"Packet read of type %d instead of SSH_FXP_INIT",
                      packet->type);
    ...
    sftp_packet_free(packet);
    reply=buffer_new();
    buffer_add_u32(reply,ntohl(3));
    if(sftp_packet_write(sftp,2,reply)==-1){
        buffer_free(reply);
        return -1;
    }
    buffer_free(reply);
...
SFTP_MESSAGE *sftp_get_message(SFTP_PACKET *packet){
    ...
    if((packet->type!=101)&&(packet->type!=102) &&
        (packet->type != 103) && (packet->type != 105)
    && (packet->type != 104)){
        ssh_set_error(packet->sftp->session,2,"get_message : unknown packet type %d\n",packet->type);
...
int sftp_init(SFTP_SESSION *sftp){
    ...
    sftp_packet_write(sftp,1,buffer);
    buffer_free(buffer);
...
SFTP_DIR *sftp_opendir(SFTP_SESSION *sftp, char *path){
    ...
    free(path_s);
    sftp_packet_write(sftp,11,payload);
    buffer_free(payload);
    while(!msg){
    ...
    switch (msg->packet_type){
        ...
        case 102:
            ...
            return dir;
        default:
            ssh_set_error(sftp->session,2,"Received message %d during opendir!",msg->packet_type);
            sftp_message_free(msg);
    }
    return NULL;
}
...
void buffer_add_attributes(BUFFER *buffer, SFTP_ATTRIBUTES *attr){
	...
	if(attr){
		if (flags & 0x00000001)
		{
			buffer_add_u64(buffer, ntohll(attr->size));
		}
		if(flags & 0x00000002){
...
SFTP_ATTRIBUTES *sftp_parse_attr(SFTP_SESSION *session, BUFFER *buf,int expectname){
    switch(session->version){
        ...
        default:
            ssh_set_error(session->session,2,"Version %d unsupported by client",session->server_version);
            return NULL;
    }
    return NULL;
}
...
SFTP_ATTRIBUTES *sftp_readdir(SFTP_SESSION *sftp, SFTP_DIR *dir){
    ...
    if(!dir->buffer){
        ...
        sftp_packet_write(sftp,12,payload);
        buffer_free(payload);
        ...
        switch (msg->packet_type){
            ...
            case 104:
                ...
                break;
            default:
                ssh_set_error(sftp->session,2,"unsupported message back %d",msg->packet_type);
                sftp_message_free(msg);
                return NULL;
        }
    }
...
static int sftp_handle_close(SFTP_SESSION *sftp, STRING *handle){
    ...
    sftp_packet_write(sftp,4,buffer);
    buffer_free(buffer);
    while(!msg){
        if(sftp_read_and_dispatch(sftp))
        /* something nasty has happened */
    ...
    switch (msg->packet_type){
        case 101:
            ...
            return err;
        default:
            ssh_set_error(sftp->session,2,"Received message %d during sftp_handle_close!",msg->packet_type);
            sftp_message_free(msg);
    }
    return -1;
}
...
SFTP_FILE *sftp_open(SFTP_SESSION *sftp, char *file, int access, SFTP_ATTRIBUTES *attr){
    ...
    sftp_packet_write(sftp,3,buffer);
    buffer_free(buffer);
    while(!msg){
        if(sftp_read_and_dispatch(sftp))
        /* something nasty has happened */
    ...
    switch (msg->packet_type){
        ...
        case 102:
            ...
            return handle;
        default:
            ssh_set_error(sftp->session,2,"Received message %d during open!",msg->packet_type);
            sftp_message_free(msg);
    }
    return NULL; 
}
...
int sftp_read(SFTP_FILE *handle, void *data, int len){
    ...
    buffer_add_u64(buffer,ntohll(handle->offset));
    buffer_add_u32(buffer,htonl(len));
    sftp_packet_write(handle->sftp,5,buffer);
    buffer_free(buffer);
    while(!msg){
    ...
    switch (msg->packet_type){
        ...
        case 103:
            ...
            if(string_len(datastring)>len){
                ssh_set_error(sftp->session,2,"Received a too big DATA packet from sftp server : %d and asked for %d",
            ...
            return len;
        default:
            ssh_set_error(sftp->session,2,"Received message %d during read!",msg->packet_type);
            sftp_message_free(msg);
            return -1;
    }
...
int sftp_write(SFTP_FILE *file, void *data, int len){
    ...
    buffer_add_u64(buffer,ntohll(file->offset));
    datastring=string_new(len);
    string_fill(datastring,data,len);
    buffer_add_ssh_string(buffer,datastring);
    free(datastring);
    if(sftp_packet_write(file->sftp,6,buffer) != buffer_get_len(buffer)){
    ...
    switch (msg->packet_type){
        case 101:
            ...
            return (err?err:len);
        default:
            ssh_set_error(sftp->session,2,"Received message %d during write!",msg->packet_type);
            sftp_message_free(msg);
            return -1;
    }
    return -1; /* not reached */
...
int sftp_rm(SFTP_SESSION *sftp, char *file) {
    ...
    buffer_add_ssh_string(buffer, filename);
    free(filename);
    sftp_packet_write(sftp, 13, buffer);
    buffer_free(buffer);
    while (!msg) {
        if (sftp_read_and_dispatch(sftp)) {
            return -1;
...
int sftp_rmdir(SFTP_SESSION *sftp, char *directory) {
    ...
    buffer_add_ssh_string(buffer, filename);
    free(filename);
    sftp_packet_write(sftp, 15, buffer);
    buffer_free(buffer);
    while (!msg) {
        if (sftp_read_and_dispatch(sftp))
        {
    ...
    if (msg->packet_type == 101) /* by specification, this command's only supposed to return SSH_FXP_STATUS */
    {
        status = parse_status_msg(msg);
        sftp_message_free(msg);
        if (!status)
        ...
        else if (status->status != 0)   /* status should be SSH_FX_OK if the command was successful, if it didn't, then there was an error */
        {
            ssh_set_error(sftp->session,1, "sftp server: %s", status->errormsg);
            status_msg_free(status);
            return -1;
...
int sftp_mkdir(SFTP_SESSION *sftp, char *directory, SFTP_ATTRIBUTES *attr) {
    ...
    free(path);
    buffer_add_attributes(buffer, attr);
    sftp_packet_write(sftp, 14, buffer);
    buffer_free(buffer);
    while (!msg) {
        if (sftp_read_and_dispatch(sftp))
            return -1;
...
int sftp_rename(SFTP_SESSION *sftp, char *original, char *newname) {
    ...
    free(newpath);
    sftp_packet_write(sftp, 18, buffer);
    buffer_free(buffer);
    while (!msg) {
        if (sftp_read_and_dispatch(sftp))
            return -1;
...
int sftp_setstat(SFTP_SESSION *sftp, char *file, SFTP_ATTRIBUTES *attr) {
    ...
    free(path);
    buffer_add_attributes(buffer, attr);
    sftp_packet_write(sftp, 9, buffer);
    buffer_free(buffer);
    while (!msg) {
        if (sftp_read_and_dispatch(sftp))
            return -1;
...
char *sftp_canonicalize_path(SFTP_SESSION *sftp, char *path)
{
	...
	free(pathstr);
	sftp_packet_write(sftp, 16, buffer);
	buffer_free(buffer);
	while (!msg)
	{
		if (sftp_read_and_dispatch(sftp))
	...
	if (msg->packet_type == 104)   /* good response */
	{
	...
	else if (msg->packet_type == 101)	/* bad response (error) */
	{
		status = parse_status_msg(msg);
		sftp_message_free(msg);
		if (!status)
...
SFTP_ATTRIBUTES *sftp_xstat(SFTP_SESSION *sftp, char *path,int param){
    ...
    ssh_set_error(sftp->session,2,"Received mesg %d during stat(),mesg->packet_type");
    sftp_message_free(msg);
    return NULL;
}

SFTP_ATTRIBUTES *sftp_stat(SFTP_SESSION *session, char *path){
    return sftp_xstat(session,path,17);
}
SFTP_ATTRIBUTES *sftp_lstat(SFTP_SESSION *session, char *path){
    return sftp_xstat(session,path,7);
}
...
SFTP_ATTRIBUTES *sftp_fstat(SFTP_FILE *file) {
    ...
    buffer_add_u32(buffer,id);
    buffer_add_ssh_string(buffer,file->handle);
    sftp_packet_write(file->sftp,8,buffer);
    buffer_free(buffer);
    while(!msg){
        if(sftp_read_and_dispatch(file->sftp))
            return NULL;
    ...
    ssh_set_error(file->sftp->session,2,"Received mesg %d during fstat(),mesg->packet_type");
    sftp_message_free(msg);
    return NULL;
}
