...
int ssh_connect_host(SSH_SESSION *session, const char *host, const char 
    ...
    if(!hp){
        --count;
        ssh_set_error(session,2,"Failed to resolve hostname %s (%s)",host,hstrerror(h_errno));
        return -1;
    }
    ...
    if(bind_addr){
        ...
        if(!hp){
            --count;
            ssh_set_error(session,2,"Failed to resolve bind address %s (%s)",bind_addr,hstrerror(h_errno));
            return -1;
        }
    }
    ...
    if(s<0){
        if(bind_addr)
            --count;
        ssh_set_error(session,2,"socket : %s",strerror(errno));
        return s;
    }
    ...
    if(bind_addr){
        ...
        if(bind(s,(struct sockaddr *)&bindsa,sizeof(bindsa))<0){
            ssh_set_error(session,2,"Binding local address : %s",strerror(errno));
            close(s);
            return -1;
        }
    }
    if(timeout){
        ...
        if(ret==0){
            /* timeout */
            ssh_set_error(session,2,"Timeout while connecting to %s:%d",host,port);
            close(s);
            return -1;
        }
        if(ret<0){
            ssh_set_error(session,2,"Select error : %s",strerror(errno));
            close(s);
            return -1;
        }
        ...
        if (ret!=0){
            ssh_set_error(session,2,"Connecting : %s",strerror(ret));
            close(s);
            return -1;
        }
        /* s is connected ? */
    ...
    if(connect(s,(struct sockaddr *)&sa,sizeof(sa))< 0){
        close(s);
        ssh_set_error(session,2,"connect: %s",strerror(errno));
        return -1;
    }
    return s;
}
...
int ssh_fd_poll(SSH_SESSION *session, int *write, int *except){
    ...
    if (select(session->fd + 1, &rdes,&wdes,&edes, &sometime) < 0) {
    	ssh_set_error(NULL,2, "select: %s", strerror(errno));
    	return -1;
    }
    if(!session->data_to_read)
...
int ssh_select(CHANNEL **channels,CHANNEL **outchannels, int maxfd, fd_set *readfds, struct timeval *timeout){
    ...
    if(rep==-1 && errno==EINTR){
        return 3; /* interrupted by a signal */
    }
    if(rep==-1){
