...
char *default_methods[]={
	"diffie-hellman-group1-sha1","ssh-dss,ssh-rsa","" "" "3des-cbc","" ""
        "3des-cbc", "hmac-sha1","hmac-sha1","none","none","","",NULL };
...
int ssh_get_kex(SSH_SESSION *session,int server_kex ){
    ...
    char *strings[10];
    int i;
    if(packet_wait(session,20,1))
        return -1;
    if(buffer_get_data(session->in_buffer,session->server_kex.cookie,16)!=16){
        ssh_set_error(session,2,"get_kex(): no cookie in packet");
        return -1;
    }
...
int set_kex(SSH_SESSION *session){
    ...
    for (i=0;i<10;i++){
        ...
        if(!client->methods[i] && i < 8){
            ssh_set_error(session,2,"kex error : did not find one of algos %s in list %s for %s",
            wanted,server->methods[i],ssh_kex_nums[i]);
            return -1;
        } else {
            if(i>=8 && !client->methods[i])
...
void ssh_send_kex(SSH_SESSION *session, int server_kex){
    ...
    buffer_add_u8(session->out_buffer,20);
...
static void build_session_id1(SSH_SESSION *session, STRING *servern, 
    ...
    ssh_print_hexa("session_id",session->next_crypto->session_id,16);
...
int ssh_get_kex1(SSH_SESSION *session){
    ...
    if((ko!=sizeof(u32)) || !host_mod || !host_exp || !server_mod || !server_exp){
        ...
        ssh_set_error(NULL,2,"Invalid SSH_SMSG_PUBLIC_KEY packet");
        if(host_mod)
            free(host_mod);
        if(host_exp)
            free(host_exp);
    ...
    /* hardcode 3des */
    if(!(supported_ciphers_mask & (1<<3))){
        ssh_set_error(NULL,2,"Remote server doesn't accept 3des");
        return -1;
    }
    packet_clear_out(session);
    buffer_add_u8(session->out_buffer,3);
    buffer_add_u8(session->out_buffer,3);
