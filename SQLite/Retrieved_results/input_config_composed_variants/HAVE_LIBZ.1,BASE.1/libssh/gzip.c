...
static z_stream *initcompress(SSH_SESSION *session,int level){
    ...
    if (status!=0)
        ssh_set_error(session,2,"status %d inititalising zlib deflate",status);
    return stream;
}

BUFFER *gzip_compress(SSH_SESSION *session,BUFFER *source,int level){
    BUFFER *dest;
    static unsigned char out_buf[4092];
    ...
    do {
        ...
        if(status !=0){
            ssh_set_error(session,2,"status %d deflating zlib packet",status);
            return NULL;
        }
        len=4092-zout->avail_out;
...
static z_stream *initdecompress(SSH_SESSION *session){
    ...
    if (status!=0){
        ssh_set_error(session,2,"Status = %d initiating inflate context !",status);
        free(stream);
        stream=NULL;
    }
    return stream;
...
BUFFER *gzip_decompress(SSH_SESSION *session,BUFFER *source){
    ...
    do {
        ...
        if(status !=Z_OK){
            ssh_set_error(session,2,"status %d inflating zlib packet",status);
            buffer_free(dest);
            return NULL;
        }
        len=4092-zin->avail_out;
        buffer_add_data(dest,out_buf,len);
        zin->next_out=out_buf;
