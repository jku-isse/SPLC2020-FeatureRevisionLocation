...
int channel_open_session1(CHANNEL *chan){
    ...
    if(session->exec_channel_opened){
        ssh_set_error(session,1,"SSH-1 supports only one execution channel. One has already been opened");
        return -1;
    }
...
int channel_request_pty_size1(CHANNEL *channel, char *terminal, int col, 
    ...
    buffer_add_u8(session->out_buffer,10);
...
int channel_change_pty_size1(CHANNEL *channel, int cols, int rows){
    SSH_SESSION *session=channel->session;
    packet_clear_out(session);
    buffer_add_u8(session->out_buffer,11);
    ...
    switch (session->in_packet.type){
        ...
        case 15:
            ...
            ssh_set_error(session,1,"pty size change denied");
            return -1;
    }
    ssh_set_error(session,2,"Received unexpected packet type %d",
            session->in_packet.type);
    return -1;
}
...
int channel_request_shell1(CHANNEL *channel){
    SSH_SESSION *session=channel->session;
    packet_clear_out(session);
    buffer_add_u8(session->out_buffer,12);
    if(packet_send(session))
        return -1;
...
int channel_request_exec1(CHANNEL *channel, char *cmd){
    ...
    packet_clear_out(session);
    buffer_add_u8(session->out_buffer,13);
...
static void channel_rcv_close1(SSH_SESSION *session){
    ...
    packet_clear_out(session);
    buffer_add_u8(session->out_buffer,33);
    packet_send(session);
}
...
int channel_write1(CHANNEL *channel, void *data, int len){
    ...
    while(len>0){
        packet_clear_out(session);
        buffer_add_u8(session->out_buffer,16);
        if(len > 32000)
            effectivelen=32000;
        else
            effectivelen=len;
