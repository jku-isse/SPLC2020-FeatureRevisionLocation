    ...
    while(cont){
        ...
        switch(session->in_packet.type){
            case 51:
                ...
                if(partial){
                    err=2;
                    ssh_set_error(session,0,"partial success, authentications that can continue : %s",c_cont);
                }
                else {
                    err=1;
                    ssh_set_error(session,1,"Access denied. authentications that can continue : %s",c_cont);
                }
                free(can_continue);
                free(c_cont);
                cont=0;
...
int ssh_userauth_none(SSH_SESSION *session,char *username){
    ...
    buffer_add_u8(session->out_buffer,50);
...
int ssh_userauth_autopubkey(SSH_SESSION *session){
    ...
    ssh_set_error(session,0,"no public key matched");
    if(id){
        pub_keys_path[0]=NULL;
        keys_path[0]=NULL;
        free(id);
...
static int kbdauth_info_get(SSH_SESSION *session){
    ...
    if(!name || !instruction || !tmp){
        ...
        ssh_set_error(session,2,"Invalid USERAUTH_INFO_REQUEST msg");
        return -1;
    }
    if(tmp)
        free(tmp); // no use
    ...
    if(nprompts>256){
        ssh_set_error(session,2,"Too much prompt asked from server: %lu(0x%.8lx)",nprompts,nprompts);
        return -1;
    }
    ...
    for(i=0;i<nprompts;++i){
        ...
        if(!tmp){
            ssh_set_error(session,2,"Short INFO_REQUEST packet");
            return -1;
        }
        session->kbdint->prompts[i]=string_to_char(tmp);
        free(tmp);
    }
    return 3; /* we are not auth. but we parsed the packet */
}
...
static int kbdauth_send(SSH_SESSION *session) {
    ...
    int i;
    packet_clear_out(session);
    buffer_add_u8(session->out_buffer,61);
...
int ssh_userauth_kbdint(SSH_SESSION *session,char *user,char *submethods){
    int err;
    if(session->version==1)
        return 1; // no keyb-interactive for ssh1
    if( !session->kbdint){
