...
#define set_status(opt, status) do { if (opt->connect_status_function) opt->connect_status_function(opt->connect_status_arg, status); } while (0)
...
char *ssh_get_banner(SSH_SESSION *session){
    ...
    while (i < 127) {
        if(read(session->fd, &buffer[i], 1)<=0){
            ssh_set_error(session,2,"Remote host closed connection");
            return NULL;
        }
        if (buffer[i] == '\r')
            buffer[i] = 0;
    ...
    ssh_set_error(session,2,"Too large banner");
    return NULL;
}
...
int ssh_analyze_banner(SSH_SESSION *session, int *ssh1, int *ssh2){
    ...
    if(strncmp(banner,"SSH-",4)!=0){
        ssh_set_error(session,2,"Protocol mismatch: %s",banner);
        return -1;
    }
    /* a typical banner is :
     * SSH-1.5-blah
...
int ssh_send_banner(SSH_SESSION *session,int server){
     char *banner;
    char buffer[128];
    banner=session->version==1?"SSH-1.5-" "libssh-0.2-dev":"SSH-2.0-" "libssh-0.2-dev";
...
static int dh_handshake(SSH_SESSION *session){
    ...
    buffer_add_u8(session->out_buffer,30);
    ...
    packet_send(session);
    free(e);
    if(packet_wait(session,31,1))
        return -1;
    pubkey=buffer_get_ssh_string(session->in_buffer);
    if(!pubkey){
        ssh_set_error(session,2,"No public key in packet");
        return -1;
    }
    dh_import_pubkey(session,pubkey);
    f=buffer_get_ssh_string(session->in_buffer);
    if(!f){
        ssh_set_error(session,2,"No F number in packet");
        return -1;
    }
    dh_import_f(session,f);
    free(f);
    if(!(signature=buffer_get_ssh_string(session->in_buffer))){
        ssh_set_error(session,2,"No signature in packet");
        return -1;
    }

    dh_build_k(session);
    packet_wait(session,21,1);
    ssh_say(2,"Got SSH_MSG_NEWKEYS\n");
    packet_clear_out(session);
    buffer_add_u8(session->out_buffer,21);
    packet_send(session);
...
int ssh_service_request(SSH_SESSION *session,char *service){
    STRING *service_s;
    packet_clear_out(session);
    buffer_add_u8(session->out_buffer,5);
    ...
    if(packet_wait(session,6,1)){
        ssh_set_error(session,2,"did not receive SERVICE_ACCEPT");
        return -1;
    }
...
int ssh_connect(SSH_SESSION *session){
  ...
  if(!session->options){
      ssh_set_error(session,2,"Must set options before connect");
      return -1;
  }
  session->client=1;
  ssh_crypto_init();
  if(options->fd==-1 && !options->host){
      ssh_set_error(session,2,"Hostname required");
      return -1;
  } 
  if(options->fd != -1)
      fd=options->fd;
  ...
  if(fd<0)
      return -1;
  do { if (options->connect_status_function) options->connect_status_function(options->connect_status_arg, 0.2); } while (0);
  session->fd=fd;
  session->alive=1;
  ...
  do { if (options->connect_status_function) options->connect_status_function(options->connect_status_arg, 0.4); } while (0);
  ...
  do { if (options->connect_status_function) options->connect_status_function(options->connect_status_arg, 0.5); } while (0);
  switch(session->version){
      case 2:
        if(ssh_get_kex(session,0)){
            return -1;
        }
        do { if (options->connect_status_function) options->connect_status_function(options->connect_status_arg, 0.6); } while (0);
        ssh_list_kex(&session->server_kex);
        if(set_kex(session)){
            return -1;
        }
        ssh_send_kex(session,0);
        do { if (options->connect_status_function) options->connect_status_function(options->connect_status_arg, 0.8); } while (0);
        if(dh_handshake(session)){
            return -1;
        } 
        do { if (options->connect_status_function) options->connect_status_function(options->connect_status_arg, 1.0); } while (0);
        session->connected=1;
        break;
    case 1:
...
void ssh_disconnect(SSH_SESSION *session){
    ...
    if(session->fd!= -1) {
        ...
        buffer_add_u32(session->out_buffer,htonl(11));
...
const char *ssh_copyright(){
    return "libssh-0.2-dev" " (c) 2003-2005 Aris Adamantiadis (aris@0xbadc0de.be)"
