...
static int bind_socket(SSH_BIND *ssh_bind,char *hostname, int port) {
    ...
    if(!hp){
        ssh_set_error(ssh_bind,2,"resolving %s: %s",hostname,hstrerror(h_errno));
        close(s);
        return -1;
    }
    ...
    if (bind(s, (struct sockaddr *) &myaddr, sizeof(myaddr)) < 0) {
        ssh_set_error(ssh_bind,2,"Binding to %s:%d : %s",hostname,port,
                strerror(errno));
	    close(s);
        return -1;
    }
...
int ssh_bind_listen(SSH_BIND *ssh_bind){
    ...
    ssh_bind->bindfd=fd;
    if(listen(fd,10)<0){
        ssh_set_error(ssh_bind,2,"listening to socket %d: %s",
                fd,strerror(errno));
        close(fd);
        return -1;
    }
...
SSH_SESSION *ssh_bind_accept(SSH_BIND *ssh_bind){
    ...
    if(ssh_bind->bindfd<0){
        ssh_set_error(ssh_bind,2,"Can't accept new clients on a "
                "not bound socket.");
        return NULL;
    }
    if(!ssh_bind->options->dsakey && !ssh_bind->options->rsakey){
        ssh_set_error(ssh_bind,2,"DSA or RSA host key file must be set before accept()");
        return NULL;
    }
    if(ssh_bind->options->dsakey){
        dsa=_privatekey_from_file(ssh_bind,ssh_bind->options->dsakey,1);
        if(!dsa)
            return NULL;
        ssh_say(2,"Dsa private key read successfuly\n");
    }
    if(ssh_bind->options->rsakey){
        rsa=_privatekey_from_file(ssh_bind,ssh_bind->options->rsakey,2);
        if(!rsa){
            if(dsa)
    ...
    if(fd<0){
        ssh_set_error(ssh_bind,2,"Accepting a new connection: %s",
                strerror(errno));
        if(dsa)
            private_key_free(dsa);
        if(rsa)
...
int server_set_kex(SSH_SESSION * session) {
    ...
    if(session->dsa_key && session->rsa_key){
        ssh_options_set_wanted_algos(options,1,"ssh-dss,ssh-rsa");
    } else {
        if(session->dsa_key)
            ssh_options_set_wanted_algos(options,1,"ssh-dss");
        else
            ssh_options_set_wanted_algos(options,1,"ssh-rsa");
    }
...
static int dh_handshake_server(SSH_SESSION *session){
    ...
    if(packet_wait(session, 30 ,1))
        return -1;
    e=buffer_get_ssh_string(session->in_buffer);
    if(!e){
        ssh_set_error(session,2,"No e number in client request");
        return -1;
    }
    ...
    buffer_add_u8(session->out_buffer,31);
