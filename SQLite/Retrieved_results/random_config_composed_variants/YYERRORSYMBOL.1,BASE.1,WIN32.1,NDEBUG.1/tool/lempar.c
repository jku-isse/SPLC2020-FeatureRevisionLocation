...
void *ParseAlloc(void *(*mallocProc)()){
  yyParser *pParser;
  pParser = (yyParser*)(*mallocProc)( sizeof(yyParser), "C:\\Users\\gabil\\Desktop\\ECCO_Work\\spls\\spls\\tests-11-02-2020\\SQLite\\sqlite\\tool\\lempar.c", 213 );
  if( pParser ){
    pParser->idx = -1;
  }
  return pParser;
...
void ParseFree(
  ...
  (*freeProc)(pParser, "C:\\Users\\gabil\\Desktop\\ECCO_Work\\spls\\spls\\tests-11-02-2020\\SQLite\\sqlite\\tool\\lempar.c", 288);
}
  ...
  do{
    ...
    if( yyact<YYNSTATE ){
      ...
      */
      ...
      if( yypParser->top->major==1 || yyerrorhit ){
        ...
        yymajor = YYNOCODE;
         ...
         while(
          yypParser->idx >= 0 &&
          yypParser->top->major != 1 &&
          (yyact = yy_find_parser_action(yypParser,1)) >= YYNSTATE
        ){
        ...
        if( yypParser->idx < 0 || yymajor==0 ){
          ...
          yymajor = YYNOCODE;
        }else if( yypParser->top->major!=1 ){
          YYMINORTYPE u2;
          u2.YYERRSYMDT = 0;
          yy_shift(yypParser,yyact,1,&u2);
        }
      }
      yypParser->errcnt = 3;
      yyerrorhit = 1;
