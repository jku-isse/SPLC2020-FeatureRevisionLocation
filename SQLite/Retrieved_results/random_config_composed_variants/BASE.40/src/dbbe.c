...
Dbbe *sqliteDbbeOpen(
  ...
  zMaster = 0;
  sqliteSetString(&zMaster, zName, "/" "sqlite_master", 0);
...
void sqliteDbbeReorganizeTable(Dbbe *pBe, const char *zTable){
  ...
  if( sqliteDbbeOpenTable(pBe, zTable, 1, &pTab)!=0 ){
    return;
  }
...
int sqliteDbbePut(DbbeTable *pTable, int nKey,char *pKey,int nData,char *pData){
  datum data, key;
  int rc;
  if( pTable->pFile==0 || pTable->pFile->dbf==0 ) return 2;
  data.dsize = nData;
  data.dptr = pData;
  key.dsize = nKey;
  key.dptr = pKey;
...
int sqliteDbbeOpenTempFile(Dbbe *pBe, FILE **ppFile){
  ...
  int rc = 0;  /* Value returned by this function */
