...
void sqliteDropTable(Parse *pParse, Token *pName){
  ...
  if( v ){
    ...
    for(pIdx=pTable->pIndex; pIdx; pIdx=pIdx->pNext){
      sqliteVdbeAddOp(v, 3, 0, 0, pIdx->zName, 0);
    }
  }
...
void sqliteCreateIndex(
  ...
  */
  if( pParse->initFlag==0 ){
    ...
    sqliteVdbeAddOp(v, 1, 1, 0, pIndex->zName, 0);
    ...
    sqliteVdbeAddOp(v, 35, pIndex->nField, 0, 0, 0);
    sqliteVdbeAddOp(v, 14, 1, 0, 0, 0);
    sqliteVdbeAddOp(v, 36, 0, lbl1, 0, 0);
    sqliteVdbeAddOp(v, 67, 0, 0, 0, lbl2);
...
void sqliteInsert(
  ...
  if( v ){
    ...
    sqliteVdbeAddOp(v, 34, pTab->nCol, 0, 0, 0);
...
void sqliteSelect(
  ...
  */
  ...
  if( pOrderBy ){
    sqliteVdbeAddOp(v, 21, 0, 0, 0, 0);
  }


  /* Identify column names
  */
  sqliteVdbeAddOp(v, 39, pEList->nExpr, 0, 0, 0);
  for(i=0; i<pEList->nExpr; i++){
    Expr *p;
    if( pEList->a[i].zName ){
      char *zName = pEList->a[i].zName;
      int addr = sqliteVdbeAddOp(v, 40, i, 0, zName, 0);
      if( zName[0]=='\'' || zName[0]=='"' ){
    ...
    if( p->op!=TK_FIELD ){
      ...
      sqliteVdbeAddOp(v, 40, i, 0, zName, 0);
    }else{
      if( pTabList->nId>1 ){
        char *zName = 0;
        ...
        sqliteVdbeAddOp(v, 40, i, 0, pTab->azCol[p->iField], 0);
      }
    }
  }
  ...
  */
  if( pOrderBy==0 ){
    sqliteVdbeAddOp(v, 41, pEList->nExpr, 0, 0, 0);
  }else{
    char *zSortOrder;
    sqliteVdbeAddOp(v, 23, pEList->nExpr, 0, 0, 0);
    ...
    sqliteVdbeAddOp(v, 24, pOrderBy->nExpr, 0, zSortOrder, 0);
    sqliteVdbeAddOp(v, 22, 0, 0, 0, 0);
  }
  ...
  */
  if( pOrderBy ){
    ...
    int addr;
    sqliteVdbeAddOp(v, 25, 0, 0, 0, 0);
    addr = sqliteVdbeAddOp(v, 26, 0, end, 0, 0);
    sqliteVdbeAddOp(v, 28, pEList->nExpr, 0, 0, 0);
    sqliteVdbeAddOp(v, 36, 0, addr, 0, 0);
    sqliteVdbeAddOp(v, 67, 0, 0, 0, end);
  }
...
void sqliteDeleteFrom(
  ...
  */
  ...
  addr = sqliteVdbeAddOp(v, 19, 0, end, 0, 0);
  if( pTab->pIndex ){
    sqliteVdbeAddOp(v, 45, 0, 0, 0, 0);
    ...
    for(i=1, pIdx=pTab->pIndex; pIdx; i++, pIdx=pIdx->pNext){
      ...
      sqliteVdbeAddOp(v, 35, pIdx->nField, 0, 0, 0);
      sqliteVdbeAddOp(v, 15, i, 0, 0, 0);
    }
  }
  sqliteVdbeAddOp(v, 7, 0, 0, 0, 0);
  sqliteVdbeAddOp(v, 36, 0, addr, 0, 0);
  sqliteVdbeAddOp(v, 20, 0, 0, 0, end);
...
void sqliteUpdate(
  ...
  */
  for(i=0; i<nIdx; i++){
    ...
    sqliteVdbeAddOp(v, 35, pIdx->nField, 0, 0, 0);
    sqliteVdbeAddOp(v, 15, i+1, 0, 0, 0);
  }
  ...
  */
  for(i=0; i<nIdx; i++){
    sqliteVdbeAddOp(v, 45, pTab->nCol, 0, 0, 0); /* The KEY */
    pIdx = apIdx[i];
    for(j=0; j<pIdx->nField; j++){
      sqliteVdbeAddOp(v, 45, j+pTab->nCol-pIdx->aiField[j], 0, 0, 0);
    }
    sqliteVdbeAddOp(v, 35, pIdx->nField, 0, 0, 0);
    sqliteVdbeAddOp(v, 14, i+1, 0, 0, 0);
  }
...
void sqliteCopy(
  ...
  v = pParse->pVdbe = sqliteVdbeCreate(pParse->db->pBe);
  if( v ){
    addr = sqliteVdbeAddOp(v, 30, 0, 0, 0, 0);
    ...
    addr = sqliteVdbeAddOp(v, 31, pTab->nCol, end, 0, 0);
    if( pDelimiter ){
    ...
    for(i=1, pIdx=pTab->pIndex; pIdx; pIdx=pIdx->pNext, i++){
      ...
      for(j=0; j<pIdx->nField; j++){
        sqliteVdbeAddOp(v, 32, pIdx->aiField[j], 0, 0, 0);
      }
      sqliteVdbeAddOp(v, 35, pIdx->nField, 0, 0, 0);
      sqliteVdbeAddOp(v, 14, i, 0, 0, 0);
    }
    sqliteVdbeAddOp(v, 36, 0, addr, 0, 0);
    sqliteVdbeAddOp(v, 67, 0, 0, 0, end);
  }
