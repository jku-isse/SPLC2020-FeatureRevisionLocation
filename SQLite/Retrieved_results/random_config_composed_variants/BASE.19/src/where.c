...
WhereInfo *sqliteWhereBegin(
  ...
  */
  ...
  nExpr = exprSplit((sizeof(aExpr)/sizeof(aExpr[0])), aExpr, pWhere);
  ...
  */
  loopMask = 0;
  for(i=0; i<pTabList->nId && i<(sizeof(aIdx)/sizeof(aIdx[0])); i++){
    int idx = aOrder[i];
  ...
  */
  for(i=0; i<pTabList->nId; i++){
    sqliteVdbeAddOp(v, 1, i, 0, pTabList->a[i].pTab->zName, 0);
    if( i<(sizeof(aIdx)/sizeof(aIdx[0])) && aIdx[i]!=0 ){
      sqliteVdbeAddOp(v, 1, pTabList->nId+i, 0, aIdx[i]->zName, 0);
    }
  }
  ...
  */
  ...
  for(i=0; i<pTabList->nId; i++){
    int j, k;
    int idx = aOrder[i];
    Index *pIdx = i<(sizeof(aIdx)/sizeof(aIdx[0])) ? aIdx[i] : 0;
    ...
    if( pIdx==0 ){
      ...
      ** scan of the table.
      */
      sqliteVdbeAddOp(v, 11, idx, brk, 0, cont);
      haveKey = 0;
    }else{
      ...
      */
      ...
      sqliteVdbeAddOp(v, 35, pIdx->nField, 0, 0, 0);
      sqliteVdbeAddOp(v, 4, pTabList->nId+i, 0, 0, 0);
      sqliteVdbeAddOp(v, 13, pTabList->nId+i, brk, 0, cont);
      if( i==pTabList->nId-1 && pushKey ){
        haveKey = 1;
      }else{
        sqliteVdbeAddOp(v, 4, idx, 0, 0, 0);
        haveKey = 0;
      }
    }
    loopMask |= 1<<idx;
...
void sqliteWhereEnd(WhereInfo *pWInfo){
  ...
  sqliteVdbeAddOp(v, 36, 0, pWInfo->iContinue, 0, 0);
  sqliteVdbeAddOp(v, 67, 0, 0, 0, pWInfo->iBreak);
  sqliteFree(pWInfo);
  return;
}
...
void sqliteExprCode(Parse *pParse, Expr *pExpr){
  ...
  int op;
  switch( pExpr->op ){
    case TK_PLUS:     op = 47;      break;
    case TK_MINUS:    op = 49; break;
    case TK_STAR:     op = 50; break;
    case TK_SLASH:    op = 51;   break;
    case TK_AND:      op = 63;      break;
    case TK_OR:       op = 64;       break;
    case TK_LT:       op = 59;       break;
    case TK_LE:       op = 58;       break;
    case TK_GT:       op = 57;       break;
    case TK_GE:       op = 56;       break;
    case TK_NE:       op = 54;       break;
    case TK_EQ:       op = 55;       break;
    case TK_ISNULL:   op = 60;   break;
    case TK_NOTNULL:  op = 61;  break;
    case TK_NOT:      op = 65;      break;
    case TK_UMINUS:   op = 62; break;
    default: break;
  }
  switch( pExpr->op ){
    case TK_FIELD: {
    ...
    case TK_INTEGER: {
      ...
      sqliteVdbeAddOp(v, 42, i, 0, 0, 0);
      break;
    }
    case TK_FLOAT: {
      int addr = sqliteVdbeAddOp(v, 43, 0, 0, 0, 0);
    ...
    case TK_EQ: {
      int dest;
      sqliteVdbeAddOp(v, 42, 0, 0, 0, 0);
      sqliteExprCode(pParse, pExpr->pLeft);
      sqliteExprCode(pParse, pExpr->pRight);
      dest = sqliteVdbeCurrentAddr(v) + 2;
      sqliteVdbeAddOp(v, op, 0, dest, 0, 0);
      sqliteVdbeAddOp(v, 48, 1, 0, 0, 0);
      break;
    }
    case TK_NOT:
    case TK_UMINUS: {
...
void sqliteExprIfTrue(Parse *pParse, Expr *pExpr, int dest){
  ...
  int op = 0;
  switch( pExpr->op ){
    case TK_LT:       op = 56;       break;
    case TK_LE:       op = 57;       break;
    case TK_GT:       op = 58;       break;
    case TK_GE:       op = 59;       break;
    case TK_NE:       op = 55;       break;
    case TK_EQ:       op = 54;       break;
    case TK_ISNULL:   op = 60;   break;
    case TK_NOTNULL:  op = 61;  break;
    default:  break;
  }
  switch( pExpr->op ){
    ...
    default: {
      ...
      sqliteVdbeAddOp(v, 37, 0, dest, 0, 0);
      break;
    }
  }
}
...
void sqliteExprIfFalse(Parse *pParse, Expr *pExpr, int dest){
  ...
  switch( pExpr->op ){
    ...
    case TK_NE:       op = 54;       break;
    case TK_EQ:       op = 55;       break;
    case TK_ISNULL:   op = 61;  break;
    case TK_NOTNULL:  op = 60;   break;
    default:  break;
  }
  switch( pExpr->op ){
    case TK_AND: {
