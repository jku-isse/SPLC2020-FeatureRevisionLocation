...
void sqliteSelect(
  ...
  */
  ...
  for(i=0; i<pEList->nExpr; i++){
    ...
    if( pEList->a[i].zName ){
      ...
      int addr = sqliteVdbeAddOp(v, 42, i, 0, zName, 0);
      if( zName[0]=='\'' || zName[0]=='"' ){
    ...
    if( p->op!=TK_FIELD ){
      ...
      sqliteVdbeAddOp(v, 42, i, 0, zName, 0);
    }else{
      if( pTabList->nId>1 ){
        char *zName = 0;
        ...
        sqliteVdbeAddOp(v, 42, i, 0, pTab->aCol[p->iField].zName, 0);
      }
    }
  }
  ...
  */
  if( distinct ){
    ...
    sqliteVdbeAddOp(v, 1, distinct, 1, 0, 0);
  }
  ...
  */
  if( distinct ){
    ...
    sqliteVdbeAddOp(v, 37, pEList->nExpr, 1, 0, 0);
    sqliteVdbeAddOp(v, 6, distinct, isDistinct, 0, 0);
    sqliteVdbeAddOp(v, 46, pEList->nExpr+1, 0, 0, 0);
    sqliteVdbeAddOp(v, 38, 0, pWInfo->iContinue, 0, 0);
    sqliteVdbeAddOp(v, 45, 0, 0, "", isDistinct);
    sqliteVdbeAddOp(v, 5, distinct, 0, 0, 0);
  }
  ...
  */
  if( pOrderBy ){
    char *zSortOrder;
    sqliteVdbeAddOp(v, 25, pEList->nExpr, 0, 0, 0);
    ...
    sqliteVdbeAddOp(v, 26, pOrderBy->nExpr, 0, zSortOrder, 0);
    sqliteVdbeAddOp(v, 24, 0, 0, 0, 0);
  }else if( isAgg ){
    int n = pEList->nExpr;
    for(i=0; i<n; i++){
      Expr *p = pEList->a[i].pExpr;
      ...
      int op, p1;
      if( n>1 ){
        sqliteVdbeAddOp(v, 48, n-1, 0, 0, 0);
      }
      if( id!=1 && p->pList && p->pList->nExpr>=1 ){
        sqliteExprCode(pParse, p->pList->a[0].pExpr);
      }
      switch( sqliteFuncId(&p->token) ){
        case 1: op = 50; p1 = 1; break;
        case 4:   op = 49;    p1 = 0; break;
        case 2:   op = 54;    p1 = 1; break;
        case 3:   op = 55;    p1 = 0; break;
      }
      sqliteVdbeAddOp(v, op, p1, 0, 0, 0);
    }
  }else{
    sqliteVdbeAddOp(v, 43, pEList->nExpr, 0, 0, 0);
  }
  ...
  */
  if( pOrderBy ){
    ...
    int addr;
    sqliteVdbeAddOp(v, 27, 0, 0, 0, 0);
    addr = sqliteVdbeAddOp(v, 28, 0, end, 0, 0);
    sqliteVdbeAddOp(v, 30, pEList->nExpr, 0, 0, 0);
