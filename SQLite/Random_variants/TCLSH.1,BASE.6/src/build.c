...
void sqliteCreateIndex(
  ...
  */
  if( pParse->initFlag==0 ){
    ...
    sqliteVdbeAddOp(v, 31, pIndex->nField, 0, 0, 0);
    sqliteVdbeAddOp(v, 14, 1, 0, 0, 0);
    sqliteVdbeAddOp(v, 32, 0, lbl1, 0, 0);
    sqliteVdbeAddOp(v, 63, 0, 0, 0, lbl2);
...
void sqliteInsert(
  ...
  if( v ){
    ...
    sqliteVdbeAddOp(v, 30, pTab->nCol, 0, 0, 0);
...
void sqliteSelect(
  ...
  */
  sqliteVdbeAddOp(v, 35, pEList->nExpr, 0, 0, 0);
  for(i=0; i<pEList->nExpr; i++){
    Expr *p;
    if( pEList->a[i].zName ){
      char *zName = pEList->a[i].zName;
      int addr = sqliteVdbeAddOp(v, 36, i, 0, zName, 0);
      if( zName[0]=='\'' || zName[0]=='"' ){
    ...
    if( p->op!=TK_FIELD ){
      ...
      sqliteVdbeAddOp(v, 36, i, 0, zName, 0);
    }else{
      if( pTabList->nId>1 ){
        char *zName = 0;
        ...
        sqliteVdbeAddOp(v, 36, i, 0, pTab->azCol[p->iField], 0);
      }
    }
  }
  ...
  */
  if( pOrderBy==0 ){
    sqliteVdbeAddOp(v, 37, pEList->nExpr, 0, 0, 0);
  }else{
    char *zSortOrder;
  ...
  */
  if( pOrderBy ){
    ...
    sqliteVdbeAddOp(v, 32, 0, addr, 0, 0);
    sqliteVdbeAddOp(v, 63, 0, 0, 0, end);
  }
...
void sqliteUpdate(
  ...
  */
  for(i=0; i<nIdx; i++){
    sqliteVdbeAddOp(v, 41, pTab->nCol, 0, 0, 0); /* The KEY */
    pIdx = apIdx[i];
    for(j=0; j<pIdx->nField; j++){
      sqliteVdbeAddOp(v, 41, j+pTab->nCol-pIdx->aiField[j], 0, 0, 0);
    }
    sqliteVdbeAddOp(v, 31, pIdx->nField, 0, 0, 0);
