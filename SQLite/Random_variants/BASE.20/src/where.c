  ...
  */
  ...
  for(i=0; i<pTabList->nId; i++){
    ...
    if( pIdx==0 ){
      ...
      ** scan of the table.
      */
      sqliteVdbeAddOp(v, 10, idx, brk, 0, cont);
      haveKey = 0;
    }else{
      ...
      */
      ...
      sqliteVdbeAddOp(v, 3, pTabList->nId+i, 0, 0, 0);
      sqliteVdbeAddOp(v, 14, pTabList->nId+i, brk, 0, cont);
      if( i==pTabList->nId-1 && pushKey ){
        haveKey = 1;
      }else{
        sqliteVdbeAddOp(v, 3, idx, 0, 0, 0);
        haveKey = 0;
      }
    }
    loopMask |= 1<<idx;
...
void sqliteWhereEnd(WhereInfo *pWInfo){
  ...
  sqliteVdbeAddOp(v, 37, 0, pWInfo->iContinue, 0, 0);
  sqliteVdbeAddOp(v, 70, 0, 0, 0, pWInfo->iBreak);
  sqliteFree(pWInfo);
  return;
}
...
void sqliteExprCode(Parse *pParse, Expr *pExpr){
  ...
  int op;
  switch( pExpr->op ){
    case TK_PLUS:     op = 48;      break;
    case TK_MINUS:    op = 50; break;
    case TK_STAR:     op = 51; break;
    case TK_SLASH:    op = 52;   break;
    case TK_AND:      op = 66;      break;
    case TK_OR:       op = 67;       break;
    case TK_LT:       op = 59;       break;
    case TK_LE:       op = 60;       break;
    case TK_GT:       op = 61;       break;
    case TK_GE:       op = 62;       break;
    case TK_NE:       op = 58;       break;
    case TK_EQ:       op = 57;       break;
    case TK_LIKE:     op = 55;     break;
    case TK_GLOB:     op = 56;     break;
    case TK_ISNULL:   op = 63;   break;
    case TK_NOTNULL:  op = 64;  break;
    case TK_NOT:      op = 68;      break;
    case TK_UMINUS:   op = 65; break;
    default: break;
  }
  switch( pExpr->op ){
    case TK_FIELD: {
      sqliteVdbeAddOp(v, 7, pExpr->iTable, pExpr->iField, 0, 0);
      break;
    }
    case TK_INTEGER: {
      int i = atoi(pExpr->token.z);
      sqliteVdbeAddOp(v, 43, i, 0, 0, 0);
      break;
    }
    case TK_FLOAT: {
      int addr = sqliteVdbeAddOp(v, 44, 0, 0, 0, 0);
    ...
    case TK_GLOB: {
      int dest;
      sqliteVdbeAddOp(v, 43, 1, 0, 0, 0);
      sqliteExprCode(pParse, pExpr->pLeft);
      sqliteExprCode(pParse, pExpr->pRight);
      dest = sqliteVdbeCurrentAddr(v) + 2;
      sqliteVdbeAddOp(v, op, 0, dest, 0, 0);
      sqliteVdbeAddOp(v, 49, -1, 0, 0, 0);
      break;
    }
    case TK_NOT:
    case TK_UMINUS: {
...
void sqliteExprIfTrue(Parse *pParse, Expr *pExpr, int dest){
  ...
  switch( pExpr->op ){
    ...
    default: {
      ...
      sqliteVdbeAddOp(v, 38, 0, dest, 0, 0);
      break;
    }
  }
}
...
void sqliteExprIfFalse(Parse *pParse, Expr *pExpr, int dest){
  ...
  int op = 0;
  switch( pExpr->op ){
    case TK_LT:       op = 62;       break;
    case TK_LE:       op = 61;       break;
    case TK_GT:       op = 60;       break;
    case TK_GE:       op = 59;       break;
    case TK_NE:       op = 57;       break;
    case TK_EQ:       op = 58;       break;
    case TK_LIKE:     op = 55;     break;
    case TK_GLOB:     op = 56;     break;
    case TK_ISNULL:   op = 64;  break;
    case TK_NOTNULL:  op = 63;   break;
    default:  break;
  }
  switch( pExpr->op ){
    case TK_AND: {
