...
static block_t block_buffer[16];            // A ring buffer for motion instfructions
...
void calculate_trapezoid_for_block(block_t *block, float entry_speed, float exit_speed) {
  ...
  unsigned char _sreg = SREG; cli();;  // Fill variables used by the stepper in a critical section
...
void planner_reverse_pass() {
  ...
  if(((block_buffer_head-block_buffer_tail + 16) & (16 - 1)) > 3) {
    block_index = (block_buffer_head - 3) & (16 - 1);
    block_t *block[5] = {
      NULL, NULL, NULL, NULL, NULL  };
    while(block_index != block_buffer_tail) { 
      block_index = (block_index-1) & (16 -1); 
      block[2]= block[1];
      block[1]= block[0];
...
void check_axes_activity() {
  ...
  if((false) && (x_active == 0)) do { if (!0) {DIO27_WPORT |= (1 << DIO27_PIN); } else {DIO27_WPORT &= ~(1 << DIO27_PIN); }; } while (0);
  if((false) && (y_active == 0)) do { if (!0) {DIO29_WPORT |= (1 << DIO29_PIN); } else {DIO29_WPORT &= ~(1 << DIO29_PIN); }; } while (0);
  if((false) && (z_active == 0)) do { if (!0) {DIO35_WPORT |= (1 << DIO35_PIN); } else {DIO35_WPORT &= ~(1 << DIO35_PIN); }; } while (0);
  if((false) && (e_active == 0)) do { if (!0) {DIO41_WPORT |= (1 << DIO41_PIN); } else {DIO41_WPORT &= ~(1 << DIO41_PIN); }; } while (0);
}
...
void plan_buffer_line(float x, float y, float z, float e, float feed_rate) {
  ...
  int next_buffer_head = (block_buffer_head + 1) & (16 - 1);
  ...
  target[2] = lround(z*axis_steps_per_unit[2]);     
  target[3] = lround(e*axis_steps_per_unit[3]); 
  ...
  block->steps_z = labs(target[2]-position[2]);
  block->steps_e = labs(target[3]-position[3]);
  ...
  //enable active axes
  if(block->steps_x != 0) do { if (0) {DIO27_WPORT |= (1 << DIO27_PIN); } else {DIO27_WPORT &= ~(1 << DIO27_PIN); }; } while (0);
  if(block->steps_y != 0) do { if (0) {DIO29_WPORT |= (1 << DIO29_PIN); } else {DIO29_WPORT &= ~(1 << DIO29_PIN); }; } while (0);
  if(block->steps_z != 0) do { if (0) {DIO35_WPORT |= (1 << DIO35_PIN); } else {DIO35_WPORT &= ~(1 << DIO35_PIN); }; } while (0);
  if(block->steps_e != 0) do { if (0) {DIO41_WPORT |= (1 << DIO41_PIN); } else {DIO41_WPORT &= ~(1 << DIO41_PIN); }; } while (0);

  float delta_x_mm = (target[0]-position[0])/axis_steps_per_unit[0];
  float delta_y_mm = (target[1]-position[1])/axis_steps_per_unit[1];
  float delta_z_mm = (target[2]-position[2])/axis_steps_per_unit[2];
  float delta_e_mm = (target[3]-position[3])/axis_steps_per_unit[3];
  ...
  int blockcount=(block_buffer_head-block_buffer_tail + 16) & (16 - 1);
  
  if ((blockcount>0) && (blockcount < (16 - 4))) {
  ...
  if(abs(block->speed_x) > max_feedrate[0]) {
    ...
    speed_factor = max_feedrate[0] / abs(block->speed_x);
    //if(speed_factor > tmp_speed_factor) speed_factor = tmp_speed_factor;
  }
  if(abs(block->speed_y) > max_feedrate[1]){
    float tmp_speed_factor = max_feedrate[1] / abs(block->speed_y);
    if(speed_factor > tmp_speed_factor) speed_factor = tmp_speed_factor;
  }
  if(abs(block->speed_z) > max_feedrate[2]){
    float tmp_speed_factor = max_feedrate[2] / abs(block->speed_z);
    if(speed_factor > tmp_speed_factor) speed_factor = tmp_speed_factor;
  }
  if(abs(block->speed_e) > max_feedrate[3]){
    float tmp_speed_factor = max_feedrate[3] / abs(block->speed_e);
    if(speed_factor > tmp_speed_factor) speed_factor = tmp_speed_factor;
  }
  ...
  if(block->steps_x == 0 && block->steps_y == 0 && block->steps_z == 0) {
    ...
    if((tmp_acceleration * block->steps_x) > axis_steps_per_sqr_second[0]) {
    ...
    if((tmp_acceleration * block->steps_y) > axis_steps_per_sqr_second[1]) {
      block->acceleration_st = axis_steps_per_sqr_second[1];
      tmp_acceleration = (float)block->acceleration_st / (float)block->step_event_count;
    }
    if((tmp_acceleration * block->steps_e) > axis_steps_per_sqr_second[3]) {
      block->acceleration_st = axis_steps_per_sqr_second[3];
      tmp_acceleration = (float)block->acceleration_st / (float)block->step_event_count;
    }
    if((tmp_acceleration * block->steps_z) > axis_steps_per_sqr_second[2]) {
      block->acceleration_st = axis_steps_per_sqr_second[2];
      tmp_acceleration = (float)block->acceleration_st / (float)block->step_event_count;
    }
  }
  ...
  if((block->steps_e == 0) || (block->steps_x == 0 && block->steps_y == 0 && block->steps_z == 0)) {
    ...
    float advance = ((axis_steps_per_unit[3]/ (0.25 * 1.7 * 1.7 * 3.14159)) * .3) * 
      (block->speed_e * block->speed_e * (0.25 * 1.7 * 1.7 * 3.14159) * (0.25 * 1.7 * 1.7 * 3.14159) / 3600.0)*65536;
  ...
  if (target[0] < position[0]) { 
    block->direction_bits |= (1<<0); 
  }
  if (target[1] < position[1]) { 
    block->direction_bits |= (1<<1); 
  }
  if (target[2] < position[2]) { 
    block->direction_bits |= (1<<2); 
  }
  if (target[3] < position[3]) { 
    block->direction_bits |= (1<<3); 
  }
...
{
  position[0] = lround(x*axis_steps_per_unit[0]);
  position[1] = lround(y*axis_steps_per_unit[1]);
  position[2] = lround(z*axis_steps_per_unit[2]);     
  position[3] = lround(e*axis_steps_per_unit[3]);     
}
