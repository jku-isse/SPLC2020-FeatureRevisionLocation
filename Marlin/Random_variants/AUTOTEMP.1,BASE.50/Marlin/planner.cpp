...
void getHighESpeed()
{
  if(degTargetHotend0()+2<190)  //probably temperature set to zero.
    return; //do nothing
  float high=0;
  char block_index = block_buffer_tail;
  ...
  float g=190+high_e_speed*1000.;
  float t=constrain(190,g,260);
  setTargetHotend0(t);
  serialprintPGM(echomagic);;
  {serialprintPGM(PSTR("highe"));;Serial.print(high_e_speed);;};
  {serialprintPGM(PSTR(" t"));;Serial.print(t);;};
