...
SREG = _sreg;;
  ...
  previous_millis_bed_heater = millis();
    ...
    {
      do { if (LOW) {PORTB |= (1 << PINB4); } else {PORTB &= ~(1 << PINB4); }; } while (0);
    }
    else 
    {
      do { if (HIGH) {PORTB |= (1 << PINB4); } else {PORTB &= ~(1 << PINB4); }; } while (0);
    }
...
void tp_init()
{
  ...
  do {DDRB |= (1 << PINB2); } while (0);


  do {DDRB |= (1 << PINB4); } while (0);
...
ISR(TIMER0_COMPB_vect)
{
  ...
  if(temp_count >= 16) // 6 ms * 16 = 96ms.
  {
    ...
    if(current_raw[1] <= bed_minttemp) {
      target_raw[1] = 0;
      do { if (0) {PORTB |= (1 << PINB4); } else {PORTB &= ~(1 << PINB4); }; } while (0);
