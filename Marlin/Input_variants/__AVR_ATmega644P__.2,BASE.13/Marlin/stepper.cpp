...
if ((out_bits & (1<<0)) != 0) {   // -direction
      do { if (true) {PORTC |= (1 << PINC7); } else {PORTC &= ~(1 << PINC7); }; } while (0);
      
      
      

      if(((bool)(PINC & (1 << PINC6))) != ENDSTOPS_INVERTING) {
        ...
        if((((bool)(PINA & (1 << PINA7))) != ENDSTOPS_INVERTING)  && (current_block->steps_x >0)){
          step_events_completed = current_block->step_event_count;
        }

    }

    if ((out_bits & (1<<1)) != 0) {   // -direction
      do { if (false) {DIO33_WPORT |= (1 << DIO33_PIN); } else {DIO33_WPORT &= ~(1 << DIO33_PIN); }; } while (0);
      
      
      

      if(((bool)(PINA & (1 << PINA5))) != ENDSTOPS_INVERTING) {
        step_events_completed = current_block->step_event_count;
      }
    ...
    if((((bool)(PINA & (1 << PINA3))) != ENDSTOPS_INVERTING) && (current_block->steps_y >0)){
        step_events_completed = current_block->step_event_count;
      }

    }

    if ((out_bits & (1<<2)) != 0) {   // -direction
      do { if (true) {DIO39_WPORT |= (1 << DIO39_PIN); } else {DIO39_WPORT &= ~(1 << DIO39_PIN); }; } while (0);
      
      
      

      if(((bool)(PINA & (1 << PINA1))) != ENDSTOPS_INVERTING) {
        step_events_completed = current_block->step_event_count;
      }
    ...
    for(char i=0; i < step_loops; i++) { // Take multiple steps per interrupt (For high speed moves) 
      ...
      if (counter_x > 0) {
        do { if (HIGH) {PORTA |= (1 << PINA6); } else {PORTA &= ~(1 << PINA6); }; } while (0);
        counter_x -= current_block->step_event_count;
        do { if (LOW) {PORTA |= (1 << PINA6); } else {PORTA &= ~(1 << PINA6); }; } while (0);
      ...
      if (counter_y > 0) {
        do { if (HIGH) {PORTA |= (1 << PINA0); } else {PORTA &= ~(1 << PINA0); }; } while (0);
        counter_y -= current_block->step_event_count;
        do { if (LOW) {PORTA |= (1 << PINA0); } else {PORTA &= ~(1 << PINA0); }; } while (0);
...
void st_init()
{
  ...
  do {DDRA |= (1 << PINA4); } while (0);
  if(!0) do { if (HIGH) {PORTA |= (1 << PINA4); } else {PORTA &= ~(1 << PINA4); }; } while (0);


  do {DDRA |= (1 << PINA2); } while (0);
  if(!0) do { if (HIGH) {PORTA |= (1 << PINA2); } else {PORTA &= ~(1 << PINA2); }; } while (0);
  ...
  do {DDRC &= ~(1 << PINC6); } while (0); 
  do { if (HIGH) {PORTC |= (1 << PINC6); } else {PORTC &= ~(1 << PINC6); }; } while (0);


  do {DDRA &= ~(1 << PINA7); } while (0); 
  do { if (HIGH) {PORTA |= (1 << PINA7); } else {PORTA &= ~(1 << PINA7); }; } while (0);


  do {DDRA &= ~(1 << PINA5); } while (0); 
  do { if (HIGH) {PORTA |= (1 << PINA5); } else {PORTA &= ~(1 << PINA5); }; } while (0);


  do {DDRA &= ~(1 << PINA3); } while (0); 
  do { if (HIGH) {PORTA |= (1 << PINA3); } else {PORTA &= ~(1 << PINA3); }; } while (0);


  do {DDRA &= ~(1 << PINA1); } while (0); 
  do { if (HIGH) {PORTA |= (1 << PINA1); } else {PORTA &= ~(1 << PINA1); }; } while (0);
  ...
  do {DDRA |= (1 << PINA6); } while (0);


  do {DDRA |= (1 << PINA0); } while (0);
